...
class TMDB(object):
    ...
    def _get_guest_session_id_path(self, key):
        return self._get_path(key).format(
            guest_session_id=self.guest_session_id) # type: ignore

    def _get_credit_id_path(self, key):
        return self._get_path(key).format(credit_id=self.credit_id) # type: ignore

    def _get_series_id_season_number_path(self, key):
        return self._get_path(key).format(series_id=self.series_id, # type: ignore
            season_number=self.season_number) # type: ignore

    def _get_series_id_season_number_episode_number_path(self, key):
        return self._get_path(key).format(series_id=self.series_id, # type: ignore
            season_number=self.season_number, # type: ignore
            episode_number=self.episode_number) # type: ignore
    ...
    def _request(self, method, path, params=None, payload=None):
        ...
        while retry:
                ...
                response = urlquick.request(
                       method, url, params=params,
                       data=json.dumps(payload) if payload else payload,
                       headers=self.headers)
                    ##   headers=self.headers, max_age=-1)
